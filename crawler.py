...
_feedback_block_cache = None

def get_feedback_examples() -> str:
    """importance_feedback의 담당자 수정 사례를 few-shot으로 분류 프롬프트에 주입.
    대시보드에서 긴급도를 수정할 때마다 누적 → 분류가 점점 담당자 기준에 수렴.
    실행당 1회만 조회 (캐시)."""
    global _feedback_block_cache
    if _feedback_block_cache is not None:
        return _feedback_block_cache
    try:
        res = sb.table('importance_feedback').select('title,user_importance') \
            .order('updated_at', desc=True).limit(20).execute()
        rows = [r for r in (res.data or []) if r.get('title') and r.get('user_importance')]
        if not rows:
            _feedback_block_cache = ''
            return ''
        lines = [
        ...
        print(f'[피드백] 분류 학습 사례 {len(rows)}건 주입')
...
def classify_urgency(title: str, content: str = '') -> str:
    ...
    try:
        ...
        resp = client.messages.create(
            ...
            max_tokens=10,
            system=_URGENCY_SYSTEM + get_feedback_examples(),

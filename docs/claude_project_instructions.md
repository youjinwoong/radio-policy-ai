...
| 3 | github-daily-crawl-daytime | `25 0-11 * * *` | 09:25~20:25 매시간 | **낮 시간 크롤러 트리거** (2026-06-17 추가) |
| 8 | briefing-trigger-0605 | `5 21 * * *` | 06:05 | **모닝 브리핑 자동 트리거** — 오늘자 없으면 dispatch (GitHub 06:00 직후 오프셋) (2026-06-17 추가) |
| 7 | briefing-trigger-0620 | `20 21 * * *` | 06:20 | 위 백업 재시도(여전히 없으면 dispatch) (2026-06-17 추가) |

- **github-daily-crawl-daytime (jobid 3)**: GitHub Actions cron이 UTC 00~11시 낮 블록(=KST 09~20시)을 best-effort로 통째 드롭하는 문제 보완책. pg_net으로 GitHub workflow_dispatch(`daily_crawl.yml`, ref=main)를 직접 호출 → 낮 시간 크롤이 GitHub 스케줄 드롭과 무관하게 매시간 실행됨. 저녁·새벽 슬롯(UTC 13/18/22)은 GitHub cron이 안정적으로 떠서 제외.
  - 인증: GitHub fine-grained PAT(repo: radio-policy-ai, Actions: Read+Write)를 **Supabase Vault에 `github_pat` 이름으로 저장** → 잡 SQL이 `vault.decrypted_secrets`에서 참조(브라우저·코드 노출 없음).
  - 의존 확장: pg_cron, pg_net, supabase_vault (모두 설치됨).
  - ⚠️ PAT 만료/회수 시 낮 트리거가 조용히 멈춤 → Vault `github_pat` 갱신 필요. 잡 수정: `select cron.alter_job(...)`, 삭제: `select cron.unschedule('github-daily-crawl-daytime')`.
...
```
cron(KST 기준, 각 슬롯 :17/:47 이중화 — GitHub 스케줄 드롭 대비):
  '17,47 0-11 * * *'     → 09~20시 매시간 :17/:47 두 번씩
  '17,47 13,18,22 * * *' → 22시·03시·07시 각 :17/:47
  '5 23 * * *'           → 08:05 + 모닝 브리핑 백업 (이 슬롯은 브리핑 백업 조건과 연동, 분 변경 금지)
실행: crawler.py (daily_crawl.yml)
모닝 브리핑 백업: github.event.schedule == '5 23 * * *' 조건, 실패 시 텔레그램 경고
  ※ 백업 스텝의 bash 스크립트는 if 블록(`fi`)과 `|| true`가 반드시 있어야 함 (2026-06-16 구문 수정 — 빠지면 스텝 항상 실패)
※ GitHub 예약(cron)은 best-effort라 지연·드롭이 잦음 → :17/:47 이중화로 누락 확률을 낮춘 것
  (정시 직후 혼잡대 :00~:10과 15분 단위(:15/:30/:45)를 피해 :09/:39 → :17/:47로 이동, 2026-06-15)
※ 낮 블록(UTC 00~11 = KST 09~20시)이 며칠째 통째 드롭되던 문제 → Supabase pg_cron 잡
  `github-daily-crawl-daytime`이 KST 09:25~20:25 매시간 workflow_dispatch로 보완 (2026-06-17, 위 'pg_cron 스케줄 잡' 참조)
...
⚠️ Actions cron 지연·누락 잦음 → 08:05 Actions 백업('5 23' 슬롯) + 09:40 로컬 백업이 최종 안전망
   (중복은 already_sent_today가 차단. 단 09:40 로컬 백업은 PC가 그 전에 켜져 있어야 작동)
```
...
- **daily_crawl.yml 브리핑 백업 스텝의 `fi`·`|| true`를 지우지 말 것** — bash if 블록 닫기·`||` 뒤 명령 보강이라 빠지면 그 스텝이 항상 구문 오류로 실패 (2026-06-16 수정)
...
- **pg_cron 잡 `github-daily-crawl-daytime`(jobid 3)·Vault `github_pat`을 지우지 말 것** — 낮 시간(KST 09~20시) 크롤이 GitHub cron 블록 드롭으로 며칠째 통째 누락되던 문제의 보완책. 삭제 시 낮 수집이 다시 멈춤 (2026-06-17 추가)
...
- **daily_crawl.yml의 '5 23 * * *' 슬롯 분(minute)을 바꾸지 말 것** — 모닝 브리핑 백업 조건(github.event.schedule == '5 23 * * *')과 연동됨. 나머지 크롤 슬롯은 :17/:47 이중화 (2026-06-15, 정시 직후 혼잡대 회피로 :09/:39에서 변경)
- **크롤 슬롯 분(minute)을 정시(:00) 또는 :15/:30/:45 같은 15분 단위로 되돌리지 말 것** — 전 세계 cron이 정시·15분 단위에 몰려 가장 혼잡함. 어중간한 :17/:47로 둬야 드롭 확률이 낮음 (2026-06-15)

...
- 전체 뷰는 과밀 방지 적응형 필터(노드 90 이하 목표까지 인용 weight 임계 2→…→30 상향, seed 엣지는 항상 유지되어 실질 하한 ~109). **안정화(stabilizationIterationsDone) 후 physics를 꺼서 노드가 계속 흔들리는 현상 방지 — 끄지 말 것**. 라벨은 배경색 strokeWidth 외곽선으로 엣지 위에서도 읽히게 함(제거 금지). 주제 포커스 뷰는 직접 이웃+계열(하위법령) 1단계만 확장(허브 법령 경유 폭발 방지).
...
- **수정 배포 시 index.html 캐시 버스터 `app.js?v=`·`styles.css?v=` 갱신 필수 (현재 `app.js?v=20260723l` / `styles.css?v=20260723a`)** — CSS 고칠 때 styles.css 버스터도 갱신해야 사용자 브라우저가 새로 받음

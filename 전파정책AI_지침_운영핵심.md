...
- 노드 상세 카드는 **주제 맥락 인식형**: 주제 포커스 중 법령 클릭 → ① 🎯 이 주제에서의 역할(엣지 설명) ② 📌 근거 조문 원문 발췌(엣지의 "제N조"를 document_chunks에서 조회 — article_no가 문서마다 '제19조(...)'/'19조(...)' 두 형태라 양쪽 매칭 필수) ③ 연결 관계는 현재 그래프 범위만. 법령 전체 요약(OKF)은 역할·근거 조문 아래에 **펼쳐서** 표시(주제 맥락이 위에 오므로 전체 요약도 함께 보이는 게 유용 — 운영자 피드백). 📌 조문 발췌는 ① 관계 설명에 "제N조"(제24~25조 범위·제N조의M 포함)가 있으면 그 조문, ② 없으면 **그 법령 안에서 주제·질문 키워드+시맨틱 하이브리드로 관련 조문을 검색**해 표시("관련 조문(키워드+의미 검색)" 라벨. 예: 주파수 분배→전파법 시행령 §10-2, 정기검사 주기→§44). 키워드(조문 제목 ×5·본문 ×1, 총칙·부칙 배제)와 문서 한정 시맨틱(`match_chunks_semantic_in_doc` RPC, getQueryEmbedding=voyage-4-lite)을 0.45:0.55로 결합. 임베딩 실패 시 키워드만으로 폴백("키워드 매칭" 라벨). **주제 포커스 중이면 직접 관계 엣지가 없는(계열로 딸려온) 노드도 주제명 키워드로 조문 검색** — 예: 주파수 분배 주제에서 전파법 시행령 클릭 시 '주파수 분배' 관련 조문. (직접 엣지 있을 때만 검색하면 계열 간접 노드는 전체 요약만 떠서 '해당 내용 아닌 전체가 나온다'는 피드백.) 원문 보기(모달)·AI 자문 프리필 버튼.
...
- **수정 배포 시 index.html 캐시 버스터 `app.js?v=`·`styles.css?v=` 갱신 필수 (현재 `app.js?v=20260723s` / `styles.css?v=20260723b`)** — CSS 고칠 때 styles.css 버스터도 갱신해야 사용자 브라우저가 새로 받음

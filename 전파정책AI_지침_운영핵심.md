...
- 노드 상세 카드는 **주제 맥락 인식형**: 주제 포커스 중 법령 클릭 → ① 🎯 이 주제에서의 역할(엣지 설명) ② 📌 근거 조문 원문 발췌(엣지의 "제N조"를 document_chunks에서 조회 — article_no가 문서마다 '제19조(...)'/'19조(...)' 두 형태라 양쪽 매칭 필수) ③ 연결 관계는 현재 그래프 범위만. 법령 전체 요약(OKF)은 역할·근거 조문 아래에 **펼쳐서** 표시(주제 맥락이 위에 오므로 전체 요약도 함께 보이는 게 유용 — 운영자 피드백). 📌 근거 조문 발췌는 "제24~25조" 같은 **범위·제N조의M**도 파싱(범위 최대 5개 조문). 원문 보기(모달)·AI 자문 프리필 버튼.
...
- **수정 배포 시 index.html 캐시 버스터 `app.js?v=`·`styles.css?v=` 갱신 필수 (현재 `app.js?v=20260723k` / `styles.css?v=20260723a`)** — CSS 고칠 때 styles.css 버스터도 갱신해야 사용자 브라우저가 새로 받음

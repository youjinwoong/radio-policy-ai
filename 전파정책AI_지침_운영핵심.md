...
- 전체 뷰: 시드 엣지 + 인용 엣지(weight≥임계, 노드 220 이하 목표로 3→…→12 상향)로 표시 노드를 정하고, **계열(family) 엣지는 양끝이 이미 포함된 경우에만** 유지(무조건 유지하면 국가회계법–시행령처럼 주변 관계가 잘린 고립 쌍이 딸려 들어와 '관계 없는 법'처럼 보였음 — 운영자 피드백). 실측 ~207 노드. **안정화(stabilizationIterationsDone) 후 physics를 꺼서 노드가 계속 흔들리는 현상 방지 — 끄지 말 것**. 라벨은 배경색 strokeWidth 외곽선으로 엣지 위에서도 읽히게 함(제거 금지, 전체 뷰는 노드가 많아 초기 fit에선 작게 보이나 physics off라 휠 확대로 읽음). 주제 포커스 뷰는 직접 이웃+계열(하위법령) 1단계만 확장(허브 법령 경유 폭발 방지).
- 노드 상세 카드는 **주제 맥락 인식형**: 주제 포커스 중 법령 클릭 → ① 🎯 이 주제에서의 역할(엣지 설명) ② 📌 근거 조문 원문 발췌(엣지의 "제N조"를 document_chunks에서 조회 — article_no가 문서마다 '제19조(...)'/'19조(...)' 두 형태라 양쪽 매칭 필수) ③ 연결 관계는 현재 그래프 범위만. 법령 전체 요약(OKF)은 역할·근거 조문 아래에 **펼쳐서** 표시(주제 맥락이 위에 오므로 전체 요약도 함께 보이는 게 유용 — 운영자 피드백). 📌 조문 발췌는 ① 관계 설명에 "제N조"(제24~25조 범위·제N조의M 포함)가 있으면 그 조문, ② 없으면 **그 법령 안에서 주제·질문 키워드로 관련 조문을 검색**해 표시("관련 조문(키워드 매칭)" 라벨. 예: 전파법 시행령의 검사 절차·주기 → 제44조 정기검사의 유효기간). 원문 보기(모달)·AI 자문 프리필 버튼.
...
- **수정 배포 시 index.html 캐시 버스터 `app.js?v=`·`styles.css?v=` 갱신 필수 (현재 `app.js?v=20260723n` / `styles.css?v=20260723a`)** — CSS 고칠 때 styles.css 버스터도 갱신해야 사용자 브라우저가 새로 받음

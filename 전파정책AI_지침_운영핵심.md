...
③ **최후 폴백 — 키워드+시맨틱 하이브리드** — 위 둘로 안 잡히면. 키워드(조문 단위 1회 집계·제목 ×5·본문 ×1·제목에 주제명 통째 포함 시 +15·총칙/부칙 배제)와 문서 한정 시맨틱(`match_chunks_semantic_in_doc` RPC, voyage-4-lite)을 0.45:0.55 결합. **키워드는 청크마다 더하지 말 것**(긴 조문 편향으로 정확한 짧은 조문이 밀림 — §14가 §10-2를 이기던 원인). 임베딩 실패 시 키워드만("키워드 매칭" 라벨). **주제 포커스 중이면 직접 관계 엣지가 없는(계열로 딸려온) 노드도 주제명 키워드로 조문 검색** — 예: 주파수 분배 주제에서 전파법 시행령 클릭 시 '주파수 분배' 관련 조문. (직접 엣지 있을 때만 검색하면 계열 간접 노드는 전체 요약만 떠서 '해당 내용 아닌 전체가 나온다'는 피드백.) 원문 보기(모달)·AI 자문 프리필 버튼.
...
- **수정 배포 시 index.html 캐시 버스터 `app.js?v=`·`styles.css?v=` 갱신 필수 (현재 `app.js?v=20260723v` / `styles.css?v=20260723b`)** — CSS 고칠 때 styles.css 버스터도 갱신해야 사용자 브라우저가 새로 받음
